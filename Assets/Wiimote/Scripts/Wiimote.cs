...
namespace WiimoteApi {
    ...
    public class Wiimote {
        ...
        public bool SetupIRCamera (IRDataType type = IRDataType.EXTENDED) {
            ...
            res = SendRegisterWriteRequest (RegisterType.CONTROL, 0xb00033, new byte[] {
                (byte) type
            });
            if (res < 0) return false;
        ...
        public int SendWithType (OutputDataType type, byte[] data) {
            ...
            if (res < -1) {
                Debug.LogError ("Incorrect Input to HIDAPI.  No data has been sent. Error ID:" + res);
                SettingsManager.error_hidapi = true;
            }
